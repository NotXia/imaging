...
\begin{itemize}
    ...
    \item Metodo naive $\min\limits_{x} \frac{1}{2} \Vert Ax-b \|_{2}^{2}$. Questo approccio è noto per essere mal condizionato.
...
\begin{center}
    ...
    \end{tabular}
\end{center}
Come in tutti i casi precedenti, la ricostruzione naive ha prodotto soluzioni completamente distanti dall realtà.\\
...
Con Tikhonov si nota che la fedeltà rispetto all'immagine reale aumenta all'aumentare di $\sigma_{\text{kernel}}$, mentre tramite variazione totale, il fenomeno è inverso, generando una ricostruzione meno accurata.
