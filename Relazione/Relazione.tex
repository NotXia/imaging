...
Si nota che, variando la dimensione del kernel fissato $\sigma_{\text{kernel}}$, il risultato ottenuto varia tra i diversi casi di piccole quantità e non appaiono particolari correlazioni.\\
...
Come in \autoref{chap:lambda}, il risultato ottenuto con naive in tutti i casi è molto instabile, 
mentre con Tikhonov vengono prodotti risultati significativi con valori di $\sigma_{\text{kernel}}$ non troppo piccoli.\\
Per la regolarizzazione tramite variazione totale, invece, a differenza dei casi precedentemente analizzati, è presente il problema di semi-convergenza in una forma molto contenuta:
\begin{figure}[H]
    \centering
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=7cm]{reale/1/1/tv_semiconvergenza.png}
        \caption{PSNR per la $1^a$ esecuzione}
    \label{fig:semiconvergenza_reale1_1}
    \end{minipage}\hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=7cm]{reale/1/2/tv_semiconvergenza.png}
        \caption{PSNR per la $2^a$ esecuzione}
...
L'unico aspetto particolare è che la semiconvergenza della regolarizzazione di Tikhonov ha assunto un comportamento simile a quello della variazione totale:
\begin{figure}[H]
    \centering
    \includegraphics[width=15cm]{reale/2/tikhonov_semiconvergenza.png}
    \caption{Variazione del PSNR per Tikhonov}
    \label{fig:semiconvergenza_reale2}
\end{figure}
...
A livello di velocità, il numero di iterazioni richiesto in forma aggregata ha prodotto una deviazione standard molto elevata e per questo tende a non essere prevedibile. 
Anche il tempo di esecuzione è solitamente stato maggiore rispetto a Tikhonov.\\
